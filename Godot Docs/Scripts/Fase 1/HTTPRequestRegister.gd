...
func _on_HTTPRequest_request_completed(result, response_code, headers, body):
	...
	if response_code == 201:
		...
		if response_code == 401:
			$Label.text = "Nome de usuario ou de jogador ja possui um cadastro"
...
func _on_Confirm():
	...
	if cond1 && cond2 && cond3:
		if UserData.confirm == UserData.password:	 
			...
			$Label.text = "Senha não confirmada!"
			push_error("Senha não confirmada!")
	else:
		$Label.text = "A quantidade de caracteres de qualquer campo deve ser >= 5 ou <= 20"
